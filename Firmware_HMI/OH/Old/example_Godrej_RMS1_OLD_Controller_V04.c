 ...
 * 
 ****************************************************************************/
#ifdef __EXAMPLE_Godrej_RMS1_OLD_Cntroller_V04__ 
...
u8 APN_NAME[20] = "airtelgprs.com\0";
u8 DEVICE_ID[15]="90000000\0"; 
u8 START_OF_THE_STRING[10] = "$#OH";
u8 VERSION[15]="21.06.04\0";
...
void CREATE_AND_WRITE_QUERY_FOR_MULTIPLE_ADRESSES(int address,char *data_array, int length);
...
void MODBUS_READ_QURIES()
{
	if(send_flag1 == 1)
	{
		...
		{
			case 0: query_ctr1 = 1 ;
					Ql_UART_Write(UART_PORT3, QUERY_01, 8 );
					query_ctr = 1;
					send_flag1 = 0;
					APP_DEBUG("\r\n<-- Query No. 1 SEND\r\n");
					break;
    ...
    else if( UART_PORT1 == port | UART_PORT2 == port | UART_PORT3 == port)
    {
        switch (msg)
        {
            ...
            {	
				...
				if(rdLen = 20)
				{
					CREATE_AND_WRITE_QUERY_FOR_MULTIPLE_ADRESSES(10,m_Read_Buffer,rdLen-10);
				}
...
void SEPARATE_MODBUS_DATA(char *modbus_write_data)
{
	...
	for(i=0; i<10; i++)		
	{
		data_array[i] = (modbus_write_data[(i*6)] - 48) + (modbus_write_data[(i*6)+1] - 48) + (modbus_write_data[(i*6)+2] - 48) + (modbus_write_data[(i*6)+3] - 48) + (modbus_write_data[(i*6)+4] - 48);
...
void CREATE_AND_WRITE_QUERY_FOR_MULTIPLE_ADRESSES(int address,char *data_array, int length)
{
	...
	query_buff[6] = (length * 2);
	
	for(i=0; i<(length*2); i++)
	{
		query_buff[i+7] = data_array[i];
	}
	...
	APP_DEBUG("\n");
	//Ql_UART_Write(UART_PORT2, tx1buf, length_of_query );
...
void CALCULATE_CRC(int address_1, int data_1)
{
	...
	APP_DEBUG("\n");
	Ql_UART_Write(UART_PORT3, tx1buf, 8 );
