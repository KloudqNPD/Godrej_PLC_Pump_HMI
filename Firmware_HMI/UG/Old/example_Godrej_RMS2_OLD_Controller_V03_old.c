...
/************************************************************************/
static u8  m_SrvADDR[20] ="59.163.219.178\0";//"1.22.124.222\0";
static u32 m_SrvPort = 8011;

static u8  m_SrvADDR_1[20] = "59.163.219.178\0";//"1.22.124.222\0";
...
void proc_main_task()
{
	...
	Ql_UART_Register(UART_PORT3, Callback_UART_Hdlr_Modbus_Port, NULL);
    Ql_UART_OpenEx(UART_PORT3, &port2_dcb);
...
void MODBUS_READ_QURIES()
{
	if(send_flag1 == 1)
	{
		...
		{
			case 1: Ql_UART_Write(UART_PORT3, QUERY_01, 8 );
					QUERY_RCV_STAGE = 1;
					QUERY_SEND_STAGE = 1 ;
					send_flag1 = 0;
					APP_DEBUG("\r\n<-- Query No. 1 SEND\r\n");
...
static void Callback_UART_Hdlr_Modbus_Port(Enum_SerialPort port, Enum_UARTEventType msg, bool level, void* customizedPara)
{
	...
	switch (msg)
    {
		case EVENT_UART_READY_TO_READ:
        {
			...
			APP_DEBUG("<--LENGTH: %d\r\n",rdLen);      
			if((UART_PORT3 == port) && (uart_flag2 == 1))
			{
				if((rdLen <= 8) && (rdLen != 7))
				{
...
void CALCULATE_PARAMETER_FROM_QUERY(char *DATA)
{
	...
	PARAMETER_01 = (DATA[0] *255) + DATA[1];
	PARAMETER_02 = (DATA[2] *255)  + DATA[3];
	PARAMETER_03 = (DATA[4] *255)  + DATA[5];
	PARAMETER_04 = (DATA[6] *255)  + DATA[7];
	PARAMETER_05 = (DATA[8] *255)  + DATA[9];
	PARAMETER_06 = (DATA[10] *255) + DATA[11];
	PARAMETER_07 = (DATA[12] *255) + DATA[13];
	PARAMETER_08 = (DATA[14] *255) + DATA[15];
	PARAMETER_09 = (DATA[16] *255) + DATA[17];
	PARAMETER_10 = (DATA[18] *255) + DATA[19];
	PARAMETER_11 = (DATA[20] *255) + DATA[21];
	PARAMETER_12 = (DATA[22] *255) + DATA[23];
	PARAMETER_13 = (DATA[24] *255) + DATA[25];
	PARAMETER_14 = (DATA[26] *255) + DATA[27];
	PARAMETER_15 = (DATA[28] *255) + DATA[29];
	PARAMETER_16 = (DATA[30] *255) + DATA[31];
	PARAMETER_17 = (DATA[32] *255) + DATA[33];
	PARAMETER_18 = (DATA[34] *255) + DATA[35];
	PARAMETER_19 = (DATA[36] *255) + DATA[37];
	PARAMETER_20 = (DATA[38] *255) + DATA[39];
...
void CREATE_AND_WRITE_QUERY_FOR_MULTIPLE_ADRESSES(int address,int *data_array, int length)
{
	...
	APP_DEBUG("\n");
	Ql_UART_Write(UART_PORT3, query_buff, length_of_query );//write 10address in single query
